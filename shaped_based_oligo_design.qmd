...
single_stem_mutants <- function(stem_df, le_seq, le_name, n_rep = 3) {
  ...
  for (stem in 1:n_stem) { # for each stem
    ...
    while (rep_count < n_rep) {
      ...
      wo <- le_seq
      oligo_list <- c(oligo_list, mutate_bases_by_idx(working_oligo = wo, idx_list = i_idx))
...
```{r making-mutations}
mutate_bases_by_idx <- function(idx_list, working_oligo, valid_seq = 21:280) {
...
wo <- t2sq
a <- mutate_bases_by_idx(idx_list = i_1, working_oligo = wo)

a <- single_stem_mutants(stem_df = z, le_seq = t2sq, le_name = "test", n_rep = 3)
```
...
combo_stem_mutants <- function(stem_df, le_seq, le_name, n_rep = 3) {
  ...
  for (c in combos) { # for each combination of stems
    ...
    while (rep_count < n_rep) {
      rep_count <- rep_count + 1
      oligo_list <- c(oligo_list, mutate_bases_by_idx(working_oligo = le_seq, idx_list = i_idx)) # mutating i
      names_list <- c(names_list, paste0(le_name,"_stem", paste(c,collapse = "-"), "i_rep", as.character(rep_count)))
      oligo_list <- c(oligo_list, mutate_bases_by_idx(working_oligo = le_seq, idx_list = j_idx)) # mutating j
